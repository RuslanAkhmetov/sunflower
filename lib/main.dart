...
void main() => runApp(const Sunflower());
    ...
    return MaterialApp(
        ...
        home: Scaffold(
          ...
          body: Center(
            ...
            children: [
              Expanded(
                child: SunflowerWidget(seeds, curveParameter),
              ),
              ...
              Text('Change curve parameter $curveParameter'),
              SizedBox(
                width: 300,
                child: Slider(
                  min: 1,
                  max: 100,
                  value: curveParameter.toDouble(),
                  onChanged: (val) {
                    setState(() => curveParameter = val.round());
                  },
                ),
              ),
              const SizedBox(
...
class SunflowerWidget extends StatelessWidget {
  ...
  final int curveParameter;
  static var phi = (math.sqrt(5) + 1) / 2;
  static final rng = math.Random();
  final int seeds;

  const SunflowerWidget(this.seeds, this.curveParameter, {super.key});
  ...
  Widget build(BuildContext context) {
    ...
    for (var i = 0; i < seeds; i++) {
      ...
      phi = (math.sqrt(curveParameter) + 1) / 2;
